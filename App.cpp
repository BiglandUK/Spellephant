...
LRESULT CALLBACK App::StaticWndProc(HWND hWnd, UINT Message, WPARAM wParam, LPARAM lParam)
{
    ...
    App* Destination = (App*)GetWindowLong( hWnd, GWL_USERDATA );
...
bool App::InitInstance(HINSTANCE hInstance, LPSTR lpCmdLine, int nCmdShow )
{
...
bool App::CreateDisplay(HINSTANCE hInstance, int nCmdShow)
{
    WNDCLASS wc = {
                    0,
                    StaticWndProc,
...
void App::Display(BackBuffer& bb)
{
    HDC hdc = bb.getDC();
    int x = bb.mOffX;
    int y = bb.mOffY;
...
void App::Wheel(short zDelta, Gdiplus::PointF& mousePos){

    pMode_->Wheel(zDelta, &mousePos);
}

PointF App::GetMousePosition() {
    POINT p;
    GetCursorPos(&p);
